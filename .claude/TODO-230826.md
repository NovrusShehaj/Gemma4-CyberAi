...
- [ ] Benchmark too small for reliable per-category signal (n=1–4/category).
- [ ] No pre-registered success/improvement criteria committed.
- [ ] No held-out test split (single set risks benchmark overfitting during iteration).
- [ ] No training-dataset schema, validators, or builder.
- [ ] No contamination checker (train↔eval overlap).
...
**Acceptance criteria.**
- [ ] `benchmark_v2.jsonl` ≥100 items, ≥6/category, all original + licensed, no HTB/THM.
- [ ] dev/test split committed; test set documented as held-out.
- [ ] `eval_success_criteria.md` committed with concrete numeric thresholds.
- [ ] contamination checker implemented + tested + CLI.
- [ ] fresh baseline on v2 committed under `experiments/`.
- [ ] all tests pass; changes committed and pushed.
...
| P0-2 | **Benchmark v2** (the NEXT MILESTONE) | Instrument must be trustworthy before training | — | See milestone acceptance | Medium | Authoring effort/time |
