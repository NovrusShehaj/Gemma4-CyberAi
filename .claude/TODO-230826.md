...
- [x] ~~No LLM-judge scorer~~ — `src/gemma_cyber/evaluation/judge.py` (86.4% calibration vs engineer-adjudicated labels; independent human verification pending).
- [x] ~~No CI to keep tests green on push~~ — `.github/workflows/ci.yml` (pytest on push/PR).
...
|---|---|---|---|---|---|---|
| P0-1 | ✅ **Answer OPEN Q1/Q2/Q3** (DONE 2026-08-24) | Gate the entire training track & licensing obligations | — | Recorded in `docs/decisions.md`: Q1 gemma4:26b dev-preferred / gemma3:4b baseline kept; Q2 no paid GPU (local→Colab→Kaggle); Q3 distribution | Low (user decision) | — |
...
| ID | Task | Why | Deps | Acceptance | Complexity | Risk |
|---|---|---|---|---|---|---|
| P1-1 | ✅ **LLM-judge scorer** (DONE 2026-08-24) | Fix keyword false-negatives (K2) before comparisons | P0-2 | `judge.py` behind `ScoreResult` interface; calibration **86.4%** (19/22) vs engineer-adjudicated labels ≥80% (independent human verification pending); `docs/judge.md` | Medium | Judge unreliability |
...
| P1-5 | ✅ **GitHub Actions CI** (`pytest`) (DONE 2026-08-24) | Prevent regressions (D2) | — | `.github/workflows/ci.yml` runs pytest on push/PR (uv, py3.12); env-safe (no models/GPU/LLM) | Low | — |
